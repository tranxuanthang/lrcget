...
const parseLrcTimestamp = (token) => {
  ...
  const fractionMs = fractionRaw.length === 2
...
export const createSyncedLinesFromPlain = (plainLyrics, existingLines = []) => {
  const plainLines = typeof plainLyrics === 'string'
    ? plainLyrics.replace(/\r\n/g, '\n').split('\n')
    : []
...
export const parseLyricsfile = (lyricsfileContent) => {
  ...
  const plainLyrics = normalizeNonEmpty(document.plain)
    || normalizeNonEmpty(stripTimestamp(syncedLyrics))
    || ''
...
export const serializeLyricsfile = ({ track, plainLyrics, syncedLines, syncedLyrics, baseDocument }) => {
  ...
  if (!isInstrumental) {
    if (lines.length === 0 && normalizedSynced && normalizeLrcText(baseLrc) === normalizeLrcText(normalizedSynced)) {
