...
const emit = defineEmits(["playTrack", "downloadLyrics", "trackCountUpdated"]);
...
const { searchValue, hideTracksWithLyrics } = useSearchLibrary();
...
const getTrackIds = async () => {
  ...
  try {
    trackIds.value = await invoke("get_track_ids", {
      ...
      withoutPlainLyrics: hideTracksWithLyrics.value,
      withoutSyncedLyrics: hideTracksWithLyrics.value,
    });
    // Emit the updated track count
    emit("trackCountUpdated", trackIds.value.length);
  } catch (error) {
    console.error(error);
  }
};
...
watch([searchValue, hideTracksWithLyrics], async () => {
  try {
    await getTrackIds();
  } catch (error) {
    console.error(error);
