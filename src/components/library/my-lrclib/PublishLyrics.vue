<template>
  <BaseModal ref="modal" :clickOutsideToClose="false">
    <div>
      ...
      <div v-else class="px-8 py-4 max-w-[500px] max-h-[60vh] rounded-lg m-4 bg-white flex flex-col gap-4">
        <div class="flex flex-col items-center">
          <div v-if="!isPublishing" class="text-brave-10 mb-4">Do you want to publish your lyrics of the song <strong>{{ track.name }} - {{ track.artistName }}</strong> to your current LRCLIB instance?</div>
          <div v-else class="text-brave-10 mb-4">Publishing your lyrics of the song <strong>{{ track.name }} - {{ track.artistName }}</strong>...</div>
