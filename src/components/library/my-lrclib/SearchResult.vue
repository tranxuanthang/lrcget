...
onMounted(async () => {
  ...
  try {
    ...
    toast.error(
      "An error occurred while searching for lyrics. Please try again.",
    );
...
const setShowingTrack = async (track) => {
  ...
  try {
    const refreshedTrack = await invoke("retrieve_lyrics_by_id", {
      id: track.id,
    });
    ...
    toast.error(
      "An error occurred while opening the lyrics. Please try again.",
    );
    console.error(error);
  } finally {
