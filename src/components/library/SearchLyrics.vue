    ...
    <div class="grow overflow-hidden flex flex-col gap-6">
      ...
      <div class="grow overflow-hidden">
        ...
        <div v-else class="flex flex-col h-full gap-2 overflow-auto">
                ...
                <div class="text-sm font-bold">
                  ...
                  <span
                    v-if="
                      Math.round(item.duration) -
                        Math.round(searchingTrack.duration) >
                      2
                    "
                  ...
                  >
                    +{{
                      humanDuration(
                        Math.abs(
                          item.duration - Math.round(searchingTrack.duration),
                        ),
                      )
                    }}
                  </span>
          ...
          <div
            v-else
            class="flex justify-center items-center h-full text-sm text-brave-60"
          >
...
const doSearchLyrics = async () => {
  ...
  try {
    if (trackId.value) {
      searchResult.value = [
        await invoke("request_by_id", { id: trackId.value }),
      ];
    } else {
    ...
    console.error(error);
    toast.error(
      error.message || "An error occurred while searching for lyrics",
    );
  } finally {
    loading.value = false;
  }
