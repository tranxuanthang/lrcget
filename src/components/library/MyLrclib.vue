<template>
  ...
  >
    ...
    <div
      class="font-bold text-xs text-brave-30 dark:text-brave-90 select-none cursor-default"
    >
      Search with LRCLIB instance:
      <span
        class="text-brave-30 dark:text-brave-90 rounded-full px-2 py-1 bg-brave-95 dark:bg-brave-5"
        >{{ lrclibInstance }}</span
      >
    </div>
    ...
    >
      <input
        ...
        autofocus
      />
      <button class="rounded-full button button-normal h-12 w-12 m-1">
        <Magnify />
      </button>
    </form>
