<template>
  <div
    ...
    :class="{
      'border-brave-95 bg-brave-99 dark:border-brave-20 dark:bg-brave-5':
        isPlaying,
  ...
  >
    ...
    >
      <div v-if="track">
        <div
          class="font-bold text-sm text-brave-20 flex items-center dark:text-brave-95"
        >
        ...
        <div class="gap-2 line-clamp-1">
          ...
          <span class="text-brave-30 h-full mx-1 flex-none dark:text-brave-90"
            >|</span
          >
          <span class="text-sm text-brave-30 transition dark:text-brave-90">{{
            track.artist_name
    ...
    <!-- Duration -->
    <div
      class="flex-none w-[10%] flex items-center justify-end p-1"
      @click="playTrack(track)"
    >
      <div
        v-if="track"
        class="text-brave-30 font-bold text-xs text-right dark:text-brave-95"
      >
    ...
    <div class="flex-none w-[15%] h-full flex justify-end items-center p-1">
      <div v-if="track" class="flex justify-end items-center gap-1">
        ...
        <button
          v-else
          @click="isPlaying ? resume() : playTrack(track)"
          class="track-button"
        >
          <Play />
        </button>
...
const isPlaying = computed(() => {
  return (
    playingTrack.value &&
    track.value &&
    playingTrack.value.id === track.value.id
  );
});
