<template>
  <div>
    ...
    />
    <PlainLyricsViewer
      v-else-if="plainLyrics && !instrumental"
      :lyrics="plainLyrics"
    />
    ...
    >
      <div class="w-full flex gap-1 justify-center items-center">
        ...
        />
        <div
          class="flex-none text-right w-12 text-xs text-brave-30 dark:text-brave-80"
        >
      ...
      <div class="flex justify-between w-full">
        <div
          class="basis-1/3 flex-1 grow-0 flex flex-col justify-center items-start gap-0.5"
        >
          <div v-if="playingTrack">
            <div
              class="text-xs font-bold text-brave-30 dark:text-brave-95 line-clamp-1"
            >
              {{ playingTrack.title }}
            </div>
          ...
          <!-- Track count display -->
          <div
            v-if="trackCount !== null"
            class="text-xs text-brave-40 dark:text-brave-90"
          >
            {{ trackCount }} track{{ trackCount !== 1 ? "s" : "" }}
            <span
              v-if="hideTracksWithLyrics"
              class="text-brave-30 dark:text-brave-80"
            >
              (without lyrics)
            </span>
          </div>
