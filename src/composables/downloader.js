...
const updateDownloadStats = () => {
  ...
  if (lastDownloadTimestamps.length >= 2) {
    ...
    downloadSpeed.value =
      timeSpanMinutes > 0 ? completedDownloads / timeSpanMinutes : 0;
  ...
  downloadLoop = async () => {
    while (isDownloading.value) {
      ...
      try {
        ...
        addLog({
          status: "error",
          message: `Failed to get track info for ID: ${trackId}`,
...
const loadHistory = () => {
  try {
    ...
    if (savedHistory) {
      ...
      if (parsed.downloadedItems)
        downloadedItems.value = parsed.downloadedItems;
