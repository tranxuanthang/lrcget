...
async fn download_lyrics(track_id: i64, app_handle: AppHandle) -> Result<String, String> {
  ...
  let lyrics = lyrics::download_lyrics_for_track(track, config.try_embed_lyrics, &config.lrclib_instance).await.map_err(|err| err.to_string())?;
  match lyrics {
...
async fn retrieve_lyrics(title: String, album_name: String, artist_name: String, duration: f64, app_handle: AppHandle) -> Result<lrclib::get::RawResponse, String> {
  let config = app_handle.db(|db: &Connection| db::get_config(db)).map_err(|err| err.to_string())?;

  let response = lrclib::get::request_raw(&title, &album_name, &artist_name, duration, &config.lrclib_instance).await.map_err(|err| err.to_string())?;

  Ok(response)
}

#[tauri::command]
async fn retrieve_lyrics_by_id(id: i64, app_handle: AppHandle) -> Result<lrclib::get_by_id::RawResponse, String> {
  let config = app_handle.db(|db: &Connection| db::get_config(db)).map_err(|err| err.to_string())?;

  let response = lrclib::get_by_id::request_raw(id, &config.lrclib_instance).await.map_err(|err| err.to_string())?;

  Ok(response)
}

#[tauri::command]
async fn search_lyrics(title: String, album_name: String, artist_name: String, q: String, app_handle: AppHandle) -> Result<lrclib::search::Response, String> {
  let config = app_handle.db(|db: &Connection| db::get_config(db)).map_err(|err| err.to_string())?;
  let response = lrclib::search::request(&title, &album_name, &artist_name, &q, &config.lrclib_instance).await.map_err(|err| err.to_string())?;
...
async fn publish_lyrics(title: String, album_name: String, artist_name: String, duration: f64, plain_lyrics: String, synced_lyrics: String, app_handle: AppHandle) -> Result<(), String> {
  ...
  let challenge_response = lrclib::request_challenge::request(&config.lrclib_instance).await.map_err(|err| err.to_string())?;
  ...
  lrclib::publish::request(&title, &album_name, &artist_name, duration, &plain_lyrics, &synced_lyrics, &publish_token, &config.lrclib_instance).await.map_err(|err| err.to_string())?;
...
async fn flag_lyrics(track_id: i64, flag_reason: String, app_handle: AppHandle) -> Result<(), String> {
  ...
  app_handle.emit_all("flag-lyrics-progress", &progress).unwrap();
  let publish_token = format!("{}:{}", challenge_response.prefix, nonce);
  lrclib::flag::request(track_id, &flag_reason, &publish_token, &config.lrclib_instance).await.map_err(|err| err.to_string())?;
