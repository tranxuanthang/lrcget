...
async fn set_config(
    ...
    let conn_guard = app_state.db.lock().unwrap();
    let conn = conn_guard.as_ref().unwrap();
    db::set_config(skip_not_needed_tracks, try_embed_lyrics, conn)
...
async fn download_lyrics(track_id: i64, app_handle: AppHandle) -> Result<String, String> {
    let track = app_handle
        ...
        .map_err(|err| err.to_string())?;
    let lyrics = lyrics::download_lyrics_for_track(track)
        .await
        .map_err(|err| err.to_string())?;
    match lyrics {
...
async fn apply_lyrics(
    ...
    let lyrics = lyrics::apply_lyrics_for_track(track, lyrics)
        .await
        .map_err(|err| err.to_string())?;
...
async fn retrieve_lyrics(
    ...
    let response = lrclib::get::request_raw(&title, &album_name, &artist_name, duration)
        .await
        .map_err(|err| err.to_string())?;
...
async fn search_lyrics(
    ...
    let response = lrclib::search::request(&title, &album_name, &artist_name)
        .await
        .map_err(|err| err.to_string())?;
...
async fn save_lyrics(
    ...
    lyrics::apply_string_lyrics_for_track(&track, &plain_lyrics, &synced_lyrics)
        .await
        .map_err(|err| err.to_string())?;
...
async fn publish_lyrics(
    ...
    app_handle
        .emit_all("publish-lyrics-progress", &progress)
        .unwrap();
    let challenge_response = lrclib::request_challenge::request()
        .await
        .map_err(|err| err.to_string())?;
