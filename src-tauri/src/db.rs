...
pub fn get_config(db: &Connection) -> Result<PersistentConfig> {
    let mut statement =
        db.prepare("SELECT skip_not_needed_tracks, try_embed_lyrics FROM config_data LIMIT 1")?;
...
pub fn set_config(
    ...
    let mut statement = db.prepare(
        "UPDATE config_data SET skip_not_needed_tracks = ?, try_embed_lyrics = ? WHERE 1",
    )?;
...
pub fn add_track(track: &fs_track::FsTrack, db: &Connection) -> Result<()> {
    ...
    let album_id = match album_result {
        ...
        Err(_) => add_album(&track.album(), artist_id, db)?,
    };
...
pub fn search_tracks(query_str: String, db: &Connection) -> Result<Vec<i64>> {
    let query = indoc! {"
      SELECT tracks.id
      FROM tracks
      JOIN artists ON tracks.artist_id = artists.id
  ...
  "};
    ...
    let mut statement = db.prepare(query)?;
    let mut rows = statement.query(params![format!("%{}%", query_str)])?;
    let mut track_ids: Vec<i64> = Vec::new();
